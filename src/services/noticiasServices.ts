...
export class NoticiasServices {
    ...
    public static getNoticiasCrypto(): Observable<any[]> {
        ...
        if (!this.API_KEY || !this.API_URL) {
            return throwError(() => new Error('La clave de la API o la URL no se encontraron en el archivo .env.'));
        }

        const query = 'crypto';
        const url = `${this.API_URL}/v2/everything?q=${encodeURIComponent(query)}&apiKey=${this.API_KEY}&pageSize=10`;
        ...
        return from(fetch(url)).pipe(
            ...
            mergeMap(response => {
                ...
                if (!response.ok) {
                    return throwError(() => new Error(`Error HTTP! estado: ${response.status}`));
                }
            ...
            catchError(error => {
                ...
                return throwError(() => new Error('No se pudieron recuperar las noticias. Por favor, inténtelo de nuevo más tarde.'));
            })
        );
    }
}